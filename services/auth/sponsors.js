...
module.exports = class InternalAuth {
    ...
    async auth_callback(req, res) {
        ...
        if (typeof req.query.action === 'undefined' || req.query.action === "login")
            return res.status(400).redirect("/sponsors/login.html?action=login");
        ...
        res.cookie('login', req.query.token, {
            maxAge: 60 * 60 * 1000,
            httpOnly: true,
        }).redirect("/sponsors/login.html?action=register");
    }
    ...
    async login_endpoint(req, res) {
        ...
        const key_object = await this.#db.get("SELECT `sponsor_uid`, `salt`, `key` FROM sponsors WHERE `username`=?;", [username]);
        ...
        const sponsor_token = jwt.sign({
            sponsor_uid: key_object[0].sponsor_uid,
            sponsor_name: username
        }, this.#jwt_key, { expiresIn: '8h'});

        res.cookie('sponsor_verification', sponsor_token, {
    ...
    async register_endpoint(req, res) {
        ...
        const key = await this.#db.get("SELECT `key` FROM sponsors WHERE `sponsor_uid`=?;", [token.uid]);
        ...
        const username_check = await this.#db.get("SELECT * FROM sponsors WHERE `username`=?;", [req.body.user]);
        ...
        const query = await this.#db.insert("UPDATE sponsors SET `username`=?, `salt`=?, `key`=? WHERE `sponsor_uid`=?;",
            [req.body.user, salt.toString('hex'), password.toString('base64'), token.uid]);

        const sponsor_token = jwt.sign({
            sponsor_uid: token.uid,
            sponsor_name: req.body.user
        }, this.#jwt_key, { expiresIn: '8h'});
